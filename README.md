...
### How it works (and what it explicitly does _not_ do)
...
- The panel starts with: _"Waiting for Marketplace data… scroll to load
  listings."_
...
- The panel shows a clear orange notice:
  _"Couldn't read listing data — Facebook may have changed format…"_
...
> **NDJSON gotcha:** Some Marketplace GraphQL responses are _line-delimited
> JSON_ (several JSON objects separated by newlines), not a single object.
...
| [`content.js`](content.js)       | **ISOLATED**    | Owns the UI panel + state; receives messages; radius filtering; local geocoder.                                                          |
| [`cities-data.js`](cities-data.js) | **ISOLATED**  | Bundled offline US city→coords table (`self.__ML_CITIES_TSV`), loaded before `content.js`. Generated by [`tools/gen-cities.js`](tools/gen-cities.js). |
| [`panel.css`](panel.css)         | —               | Self-contained styling, all classes prefixed `ml-`.                                                                                      |

### Geocoding & the bundled city dataset

Listings expose only a **city name**, so distance is computed by geocoding the
city to coordinates and comparing to the search center (Haversine). To keep this
fast, `content.js` first looks the city up in a bundled offline table
(`cities-data.js`, ~0.9 MB, ~29k US places); only misses fall back to the
Nominatim service in `background.js` (serialized to ~1 req/sec, cached in
`chrome.storage.local`). This avoids the per-city network round-trip that
otherwise throttles filtering when scrolling.

**Dataset source / license:** built from the [GeoNames](https://www.geonames.org/)
US dump (`US.zip`, populated places with population ≥ 100), which is licensed
**CC-BY 4.0**. To regenerate after a GeoNames update:

```
# download https://download.geonames.org/export/dump/US.zip, unzip US.txt
node tools/gen-cities.js US.txt cities-data.js 100
```

### Why `hook.js` runs in the page's MAIN world

Content scripts run in an _isolated_ JavaScript world and **cannot see or hook
